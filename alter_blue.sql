...
ALTER TABLE Worklog
ADD CONSTRAINT WORKLOG_DATE_START_CHECK CHECK (EXTRACT(YEAR FROM date_start) = EXTRACT(YEAR FROM CURRENT_DATE));
...
ALTER TABLE Worklog
ADD CONSTRAINT WORKLOG_TOTAL_TIME_CHECK CHECK (total_time >= '00:00:00' AND total_time <= '24:00:00');

ALTER TABLE Worklog
ADD CONSTRAINT WORKLOG_OVERTIME_CHECK CHECK (overtime >= '00:00:00' AND overtime <= '24:00:00');

ALTER TABLE Worklog
ADD CONSTRAINT WORKLOG_UNDERTIME_CHECK CHECK (undertime >= '00:00:00' AND undertime <= '24:00:00');
...
ADD CONSTRAINT WEEKLY_REP_DATE_CHECK CHECK (EXTRACT(YEAR FROM date_create) = EXTRACT(YEAR FROM CURRENT_DATE));
...
ALTER TABLE Absence
ADD CONSTRAINT ABSENCE_DATE_START_CHECK CHECK (date_start <= date_end AND date_start >= '2010-01-01');

ALTER TABLE Absence
ADD CONSTRAINT ABSENCE_DATE_END_CHECK CHECK (date_start <= date_end AND date_end >= '2010-01-01');

ALTER TABLE Absence
ADD CONSTRAINT ABSENCE_TYPE_CHECK CHECK (type ~ '^[a-zA-Zа-яА-Я0-9,.\(\)/ -]*$');

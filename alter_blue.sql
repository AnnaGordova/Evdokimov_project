...
ALTER TABLE Absence
ADD CONSTRAINT ABSENCE_DATE_START_CHECK CHECK (EXTRACT(YEAR FROM date_start) = EXTRACT(YEAR FROM CURRENT_DATE) AND date_start <= date_end);

ALTER TABLE Absence
ADD CONSTRAINT ABSENCE_DATE_END_CHECK CHECK (EXTRACT(YEAR FROM date_end) = EXTRACT(YEAR FROM CURRENT_DATE) AND date_start <= date_end);
